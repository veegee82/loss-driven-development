...
**Thinking-levels auto-dispatch (v0.10.1+, v0.11.0 display):** every non-trivial task is auto-scored onto a 5-step rigor ladder (L0 reflex → L4 method). The scorer is deterministic (no LLM call), reads the task text, and announces the single-line `Dispatched: L<n>/<name>[ · creativity=<value>] (signals: …)` in the trace header. The prior second `mode: architect, creativity: …` line is gone — mode (reactive / architect) is a pure function of level (L0–L2 ⇒ reactive, L3/L4 ⇒ architect) and no longer user-facing. Zero-config by default; overridable with a single inline token (`LDD+`, `LDD++`, `LDD=max`, `LDD[level=Lx]`) or natural-language bump phrases (`"take your time"`, `"denk gründlich"`). Upward-biased on ties. Full reference: [`docs/ldd/thinking-levels.md`](./docs/ldd/thinking-levels.md). Implementation: [`scripts/level_scorer.py`](./scripts/level_scorer.py).
...
| `architect-mode` | **opt-in** | discipline (5-phase protocol) | Greenfield design / architecture / structural-decomposition tasks; reached through the L3 / L4 presets of the thinking-levels auto-dispatch (see [`docs/ldd/thinking-levels.md`](./docs/ldd/thinking-levels.md)), or explicitly via inline `LDD[level=L3]:` flag / `/ldd-architect` command / trigger phrases. Agent echoes the dispatch source and level in the single-line trace header |
