...
Drop it onto Claude Web's Skills upload area (or Claude Desktop's Skill import). The bundle is one consolidated skill — the `using-ldd` dispatcher becomes `SKILL.md` and the other 14 skills become `references/*.md` loaded on demand via progressive disclosure. (`host-statusline` is Claude-Code-specific and is not included in the web bundle; all other skills are.) Trigger with the `LDD:` prefix or any of the natural-language signals documented in `SKILL.md`.
...
## What Δloss_bundle means

The hero badge on this README (`Δloss_bundle = 0.561`) is the bundle's single measured-effect number. This section explains exactly what it measures, how, and how to reproduce it.

**Definition.** For each skill we run the same pressure scenario twice: once with the skill loaded (`GREEN`) and once with it masked out (`RED`). A human rubric counts how many of the listed rules the response breaks. The per-skill loss is `Δloss_normalized = (RED_violations − GREEN_violations) / rubric_max` — the fraction of avoidable violations the skill actually removes. The bundle number is the arithmetic mean of those per-skill values across every skill that is measurement-eligible:

```
Δloss_bundle = (1/N) · Σ_skill (RED − GREEN) / rubric_max
```

**Target and current value.** Target `≥ 0.30` (each skill, on average, removes at least 30 % of the avoidable violations). **Measured value: `Δloss_bundle = 0.561`** across all 11 measurement-eligible skills — target met with a substantial margin.

**Where the data lives.** Every number is backed by artifacts on disk:

| Layer | File | Purpose |
|---|---|---|
| **Raw** | `tests/fixtures/<skill>/runs/<timestamp>/score.md` | Human-authored rubric scoring per run (RED + GREEN counts, one row per rubric item) |
| **Manifest** | [`tests/fixtures/loss-bundle-manifest.json`](./tests/fixtures/loss-bundle-manifest.json) | Which skills count into the bundle, which run is canonical per skill |
| **Aggregate snapshot** | [`tests/fixtures/aggregate.json`](./tests/fixtures/aggregate.json) | Machine-readable bundle + per-skill view; regenerated by the compute script |
| **Per-skill table** | [`tests/README.md`](./tests/README.md#current-measurements) | Human-readable per-skill table + caveats (partial contamination, direct-API, inter-reviewer variance) |
| **Formal definition** | [`evaluation.md`](./evaluation.md) | Loss function, rubric design, test pyramid |

**Reproduce it in one command.** The aggregate is deterministically recomputable from the fixtures:

```bash
python3 scripts/compute-loss-bundle.py
# → table with per-skill Δloss_norm + bundle mean + target verdict, exit 0 iff target met
```

Add `--json` for the machine-readable view, or `--write` to refresh the `tests/fixtures/aggregate.json` snapshot.

**Doc-drift gate.** Every citation site (this README's badge and paragraph, `evaluation.md`, `tests/README.md`, `plugin.json`, `SUBMISSION.md`) is checked against the computed value by [`scripts/check-loss-bundle-docs.py`](./scripts/check-loss-bundle-docs.py). The pre-commit hook at [`.githooks/pre-commit`](./.githooks/pre-commit) runs this check whenever a fixture or a citation file is staged — a number mismatch blocks the commit with an actionable diff. The full maintainer workflow (when to re-measure, which files to touch) lives in [`CLAUDE.md`](./CLAUDE.md) § "Δloss_bundle — update policy".

**What is NOT aggregated into Δloss_bundle.** The `architect-mode-auto-dispatch`, `thinking-levels`, and `using-ldd-trace-visualization` fixtures test specific behaviors (dispatch signals, level-scorer boundaries, trace rendering) and are **not** aggregated into the bundle number. They live alongside the 11 aggregated skills under `tests/fixtures/` and have their own per-fixture verdicts, but by design they do not influence the bundle mean.
