...
See also:
- `../evaluation.md` — the formal loss function per skill
- (diagram omitted in web bundle) — the four-axis picture: code (θ), deliverable (y), method (m), thought (t)
- (diagram omitted in web bundle) — all four loops at a glance: Inner / Refinement / Outer on the code-axis trunk plus CoT on the thought-axis branch
- (diagram omitted in web bundle) — the CoT loop's per-step protocol
- (diagram omitted in web bundle) — the same task ending two different ways
- (diagram omitted in web bundle) — how local-optimal commits compose into global incoherence
...
- **Greedy CoT applied to a verifiable multi-step task** → groupthink at every branch; error surfaces only at the final answer, without a per-step gradient to guide the next chain. See [`./dialectical-cot.md`](./dialectical-cot.md).
...
Full per-type spec in [`../SKILL.md`](../SKILL.md) § "Loss-types — how to display the loss number".
...
4. (diagram omitted in web bundle) — the top-level picture; then `four-loops.svg` (loop-nesting view across all four axes) and `dialectical-cot.svg` (CoT per-step protocol).
5. `./loop-driven-engineering.md` — the inner loop, the entry point.
6. `./root-cause-by-layer.md` + `./loss-backprop-lens.md` — the gradient mechanics.
...
**SGD analog**: second-order / Hessian-probing. Combined with memory (first moment), it approaches a Newton-style update: *confidence(action) ∝ memory_likelihood × dialectical_likelihood × prior*. See (diagram omitted in web bundle).
...
**SGD analog**: learning-rate scheduling via a posterior-vs-prior error. Poor calibration means the optimizer's internal model of the loss landscape is wrong; the response is not bigger steps, but re-inspecting the model. See (diagram omitted in web bundle).

### 8.5 Why none of this modifies `L(θ)`

Each instrument is subject to the bias-invariance principle (§3.11 of [`./theory.md`](./theory.md)). Memory cannot turn a regressive edit into a progressive one; primers cannot invert the sign of Δloss; calibration adjusts predictions, not observations. The loss function remains externally specified by the rubric. All navigation happens in the *search* layer; the *objective* stays pure.
