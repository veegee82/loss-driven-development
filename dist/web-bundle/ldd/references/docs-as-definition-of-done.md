...
In [Gradient Descent for Agents](./theory.md), this skill is **the regularizer** that applies on all [four gradients](./convergence.md). It closes every loop: the inner loop isn't closed until code + tests + docs land together, the refinement loop isn't closed until the updated deliverable has its doc-level mental model re-synced, the outer loop isn't closed until the evolved skill is mirrored in the methodology docs, and a CoT chain closed against ground truth contributes to a running calibration record that is itself a documented artifact. Docs describe code **exactly, but at the conceptual level**. A reader of the markdown must get a faithful, current mental model of the code without reading the code.
