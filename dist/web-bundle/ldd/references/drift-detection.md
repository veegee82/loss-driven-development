...
In [Gradient Descent for Agents](./theory.md), this skill is the outer-loop sibling of [`method-evolution`](./method-evolution.md): `method-evolution` moves `m = skills/rubrics` in response to patterns; `drift-detection` surfaces the patterns worth moving against. Together they own the **method axis**. The complementary per-commit regularizer is [`docs-as-definition-of-done`](./docs-as-definition-of-done.md) — it prevents drift being introduced; this skill finds drift that snuck through anyway.
...
See [`./convergence.md`](./convergence.md) §4 for the drift taxonomy.
