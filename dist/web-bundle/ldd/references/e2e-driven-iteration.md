...
See [`./convergence.md`](./convergence.md) §5 for how this skill implements the loss trace across the test pyramid.
...
**Do not skip step 6** — the user needs to see the loss descend live, per-iteration. v0.5.1 makes this a hard step, not an optional summary at task end. Running `python -m ldd_trace append ...` appends to `.ldd/trace.log` AND prints the full trace block (sparkline + chart + per-iteration line) — one command satisfies both mandates. If the tool is unavailable, render the trace block manually per `../SKILL.md` §"Loss visualization".
