...
**This skill is the second gradient of [Gradient Descent for Agents](./theory.md).** The parameter is `y` = the deliverable (doc / diff / design / report / module). The loss is `critique defects + gate rejections + eval deltas`. The gradient is derived deterministically from the previous output — not imagined. Siblings: inner-loop skills move `θ = code`, [`method-evolution`](./method-evolution.md) moves `m = skills`, [`dialectical-cot`](./dialectical-cot.md) moves `t = reasoning chain`.
...
This is the skill-level equivalent of `awp refine` (AWP's normative refinement mode). See [`./convergence.md`](./convergence.md) §1 for the four-loop model.
...
| Multi-step reasoning with verifiable answer | CoT loop ([`dialectical-cot`](./dialectical-cot.md)) | `t` = the reasoning chain. Gate each step. |
