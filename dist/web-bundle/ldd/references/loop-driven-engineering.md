...
The [thinking-levels auto-dispatch](./thinking-levels.md) runs *before* this skill and picks the rigor level (L0…L4) — that is the step-size scheduler for the whole optimizer. This skill runs *inside* the chosen level and orchestrates the actual descent.
...
See [`./convergence.md`](./convergence.md) for the four-loop model in full, (diagram omitted in web bundle) for the top-level picture, and (diagram omitted in web bundle) for the loop-nesting view across all four axes.
