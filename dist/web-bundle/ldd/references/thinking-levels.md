...
> Authoritative reference for the 5-level auto-dispatch system. Reference spec: `../superpowers/specs/2026-04-22-ldd-thinking-levels-design.md`. Implementation: [`./level_scorer.py`](./level_scorer.py). Skill integration: [`../SKILL.md`](../SKILL.md) § Auto-dispatch: thinking-levels.
...
Deterministic pure function, implemented in [`./level_scorer.py`](./level_scorer.py). No LLM call.
...
Full list and semantic-dedup rule: [`../SKILL.md`](../SKILL.md) § Override syntax.
...
When the agent systematically under-scores tasks and ships symptom-patches where higher levels would have caught the structural origin, method-evolution auto-triggers. See [`./method-evolution.md`](./method-evolution.md) §"Automatic trigger: thinking-levels scorer weight adjustment" for the exact condition and rollback rules.
...
Nine scenarios under `../../tests/fixtures/thinking-levels/`:
...
The scorer's unit tests in `../../scripts/test_level_scorer.py` include an end-to-end block that runs all 9 fixtures through `score_task` and verifies the expected level, source, and (where relevant) creativity.
