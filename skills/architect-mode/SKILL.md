...
The auto case is implicit — no `auto-level` keyword. When the header carries `user-explicit` / `user-bump` / `user-override-down` in the parenthetical, the dispatch came from an inline flag / command / trigger phrase / natural-language bump. See [`../../skills/using-ldd/SKILL.md`](../../skills/using-ldd/SKILL.md) § "Design-phase (L3/L4) variant of the trace block" for the full format.
